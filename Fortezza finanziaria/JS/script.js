...
(function initActivationForm() {
  ...
  /* ── Assicura che la schermata successo sia nascosta al caricamento ── */
  ...
  fieldCode.addEventListener('blur', () => {
    if (fieldCode.value) fieldCode.value = normalizeCode(fieldCode.value);
  });

  /* ── Validazione al blur ── */
  fieldName.addEventListener('blur', () => {
    const val = fieldName.value.trim();
    if (!val) setFieldState(fieldName, errorName, 'error', 'Inserisci il tuo nome');
    else if (val.length < 2) setFieldState(fieldName, errorName, 'error', 'Il nome è troppo corto');
    else setFieldState(fieldName, errorName, 'valid');
  });

  fieldEmail.addEventListener('blur', () => {
    const val = fieldEmail.value.trim();
    if (!val) setFieldState(fieldEmail, errorEmail, 'error', 'Inserisci la tua email');
    else if (!isValidEmail(val)) setFieldState(fieldEmail, errorEmail, 'error', 'Email non valida');
    else setFieldState(fieldEmail, errorEmail, 'valid');
  });
  ...
  function validateAll() {
    ...
    if (!name || name.length < 2) {
      setFieldState(fieldName, errorName, 'error', name ? 'Il nome è troppo corto' : 'Inserisci il tuo nome');
      isValid = false;
    } else {
  ...
  btnActivate.addEventListener('click', async () => {
    ...
    if (!VALID_CODES.has(code)) {
      setLoading(false);
      setFieldState(fieldCode, errorCode, 'error', 'Codice non riconosciuto. Controlla l\'ultima pagina del libro.');
      fieldCode.focus();
      return;
    }
  ...
  if (btnReset) {
    btnReset.addEventListener('click', () => {
      ...
      sessionStorage.removeItem('ff_activated'); // AGGIUNGI QUESTA RIGA PER RIMUOVERE LO STATO DI ATTIVAZIONE
...
document.getElementById('stepSuccess').style.display = 'none';
      ...
      [fieldName, fieldEmail, fieldCode].forEach(el => el.classList.remove('is-valid', 'is-error'));
      [errorName, errorEmail, errorCode, errorPrivacy].forEach(el => { el.textContent = ''; });
...
// Ripristino sessione attiva
  const savedName = sessionStorage.getItem('ff_activated');
  if (savedName) {
